...
const MessengerController = {
    ...
    getMessages: async(req, res, next) => {
        try {
            // console.log('nameAuthor: ',req.params.nameAuthor)
            const getUserCurrent = await User.findById(req.data._id)
            const getInfoCurrent = await Info.find({username: getUserCurrent.username})

            const infoFriend = await Info.find({username: req.params.nameAuthor})
            const getIdUser = await User.find({username: req.params.nameAuthor})
